...
public class SphereSpawnerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.1f, radius = 633f, color = new Color(0.7f, 0.55f, 0.4f), atmosphericProperties = new Vector2(12f, 30f), atmosphereCol = new Color(0.78f, 0.75f, 0.73f) });
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.0001f, radius = 17f, color = new Color(0.7f, 0.65f, 0.55f), atmosphericProperties = Vector2.zero, pos = Vector3.back * 2000f, momentum = Vector3.right * 0.009f });
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.00019f, radius = 21f, color = new Color(0.5f, 0.45f, 0.42f), atmosphericProperties = Vector2.zero, pos = Vector3.back * 3000f, momentum = Vector3.right * 0.0083f });
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.00015f, radius = 20f, color = new Color(0.46f, 0.53f, 0.6f), atmosphericProperties = new Vector2(1f, 0.01f), pos = Vector3.back * 4500f, momentum = Vector3.right * 0.0064f });
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.00009f, radius = 16f, color = new Color(0.48f, 0.5f, 0.54f), atmosphericProperties = Vector2.zero, pos = Vector3.back * 7000f, momentum = Vector3.right * 0.005f });
        raymarcher.spheresToRender.Add(new SphereObject() { mass = 0.00069f, radius = 68f, color = new Color(0.45f, 0.6f, 0.8f), atmosphericProperties = Vector2.one, pos = Vector3.right * 14500f + Vector3.down, momentum = Vector3.back * 0.001f, atmosphereCol = new Color(0.45f, 0.6f, 0.8f) });
